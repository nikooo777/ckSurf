...
public Action Say_Hook(int client, const char[] command, int argc)
{
	...
	if (IsValidClient(client))
	{
		...
		//empty message
		//check for null first, it costs less
		if(!sText[0] || StrEqual(sText, " "))
		{
			return Plugin_Handled;		
		}
...
//to avoid compiler warnings on variable shadowing
...
public Action Event_JoinTeamFailed(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if(!client || !IsClientInGame(client))
		return Plugin_Continue;
	EJoinTeamReason m_eReason = view_as<EJoinTeamReason>(GetEventInt(event, "reason"));
